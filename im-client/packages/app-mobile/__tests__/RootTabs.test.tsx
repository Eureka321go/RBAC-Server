...
test('declares the three authenticated tabs in product order', () => {
  expect(ROOT_TAB_ITEMS.map(item => item.label)).toEqual([
    '聊天',
    '通讯录',
    '我的',
  ]);
...
test('renders all tabs with themed, safe-area-aware navigation options', async () => {
  ...
  const navigatorProps = mockNavigatorProps as unknown as Record<
    string,
    unknown
  >;
  ...
  expect(screenOptions.tabBarStyle[1]).toMatchObject({
    height: 82,
    paddingBottom: 18,
  });
  expect(screenOptions.tabBarButton).toEqual(expect.any(Function));
  ...
  expect(
    firstOptions.tabBarIcon({ color: '#66f', size: 24, focused: true }),
  ).toBeTruthy();
});
