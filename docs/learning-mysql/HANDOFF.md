...
| `03-EXPLAIN与执行计划.md` | type/key/rows/Extra 逐列读、索引失效实测 | ⬜ 未开始 |
| `04-JOIN与查询优化.md` | NLJ/BNL/hash join、驱动表、子查询 vs JOIN、DISTINCT 代价 | ⬜ 未开始 |
...
| **练习册 01 · §四 + 自检 2/4** | 桥表**不带 `deleted`** → 每条碰它的 JOIN 必须自己过滤主表 `deleted=0`，漏了出幽灵菜单；且"授权行加 `deleted` 会重蹈 `uk_user_username` 的洞"——**和 `00` 自检 4 是同一个洞**，说了 `07` 算总账 | `04`（演示怎么炸）+ `07`（算账） |
| **练习册 01 · D组 17-18 答案** | **`GROUP BY` 不会产出 0 行**（role 3 直接消失）→ 要 `LEFT JOIN` + `COUNT(列名)` 才补得回来。**已明说"这才是 `04` 讲 JOIN 的第一个真实动机"** | `04` |
...
另：`02` §一明确标注了**树高 3 层是「算出来的、不是测出来的」**（`size - n_leaf_pages` 含段内预留页，不能当非叶子页数）。**`03` 别不小心把这个差值说成实测值。**
