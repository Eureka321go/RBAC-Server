...
- [ ] 查询 `status = ENABLED` 的部门与用户，各固定一次数据库查询。
- [ ] 部门输出 `id/parentId/name/sortOrder`，成员输出 `userId/deptId/username/displayName/avatar`。
- [ ] 展示名按“非空 nickname → username”计算并稳定排序。
- [ ] Controller 不加 `@PreAuthorize`，依赖全局 JWT 登录保护。
- [ ] 运行 `mvn -f backend/pom.xml -Dmaven.test.skip=true package` 并提交 `feat(im): add complete contact directory endpoint`。
...
- [ ] 在 `ConversationService` 批量查询 ID→展示名，空 ID 集合零查询。
- [ ] `listMembers` 一次批量补齐所有成员 `displayName`，不得逐成员查询。
- [ ] `postSystem` 固化 `operatorName` 和与 `targetIds` 对齐的 `targetNames`。
- [ ] 客户端格式化优先消息内姓名，再使用调用方提供的通讯录映射，最后回退 `用户 #ID`。
- [ ] 运行后端跳过测试构建和两套 TypeScript 类型检查，提交 `fix(im): include names in group system messages`。
...
- [ ] SDK 校验 `Result.code` 和两个数组后返回通讯录。
- [ ] RN SDK 工厂装配 `contacts`，core 保持无平台依赖。
- [ ] 树模型生成根部门、子部门索引、直属成员索引、所有后代成员索引和姓名索引。
- [ ] 父节点缺失、自环或多节点环均安全提升为根节点，不允许递归死循环。
- [ ] 运行两套 TypeScript 类型检查并提交 `feat(im-client): add hierarchical contact directory model`。
...
- [ ] 选择器展示面包屑、直属子部门、直属成员、后代可选人数和部门全选/取消。
- [ ] 多选模式排除当前用户并禁用已有群成员；单选模式点击成员立即返回。
- [ ] 建群页和群加人弹窗改用同一选择器，保留手工 userId 输入。
- [ ] 单聊页改用层级单选，接口失败时仍可手工输入 userId。
- [ ] 运行移动端 TypeScript 类型检查并提交 `feat(im-client): select contacts by department hierarchy`。
...
- [ ] 群聊聚焦时并行获取群详情、成员与通讯录，构建 ID→姓名映射供旧 SYSTEM 消息使用。
- [ ] 群详情成员名称优先服务端 `displayName`，再回退通讯录，最后回退 ID。
- [ ] 运行后端跳过测试构建、两套 TypeScript 类型检查、core 平台依赖和 `git diff --check`。
- [ ] 勾选计划并记录未运行自动化测试，提交 `docs(im): complete address book implementation plan`。
- [ ] 输出多级部门、全选、禁用、旧/新 SYSTEM 姓名与失败兜底手测清单。
