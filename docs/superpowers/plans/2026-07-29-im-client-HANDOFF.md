...
本文记录 `feat/im` 分支截至 2026-07-30 的可运行状态。新窗口可直接继续开发，不需要重新梳理单聊、离线同步和双模拟器环境。语音消息、链接卡片、消息引用与富媒体长按已由用户手动验收通过；消息免打扰、会话列表时间、消息状态布局与常用 Emoji 面板已完成，等待用户手动验收。
...
- 移动端支持从单聊“会话设置”或群聊“群设置”开启、取消免打扰，并在列表展示静音图标；未读数与 @ 提醒保持不变
- 服务端与 SDK 使用显式 `lastMsgTs` 记录、迁移并前向合并真实最新消息时间，不再展示全量同步时刻
- 会话列表将未读徽标放在头像右上，时间固定在右上，免打扰图标固定在右下，并按最新消息时间排序
- 己方发送中、上传中、失败和撤回中状态互斥展示在气泡左侧；空 `seq` 不再被误判为撤回中
- 聊天输入框内置常用 Emoji 入口，面板与系统键盘互斥，支持光标插入、选区替换、连续点击和 Unicode 字素退格
- HTTP 请求收到 401 时使用安全存储中的轮换式 refresh token 单飞刷新，并将原请求最多重试一次；刷新失败会清理令牌并返回登录页
...
| --- | --- |
| `ab7d6af` | 自动刷新访问令牌 |
| `929e1df` | 接入聊天 Emoji 输入 |
| `873d8ee` | 实现常用 Emoji 面板 |
| `b4244c5` | 支持 Emoji 光标编辑 |
| `7789b6f` | 补充 Emoji 连续编辑计划 |
| `6c56e2f` | 规划常用 Emoji 面板 |
| `31fc501` | 设计常用 Emoji 面板 |
| `42c10e7` | 收紧聊天输入栏操作区 |
| `048d41a` | 收紧消息与输入栏间距 |
| `ccfa5db` | 统一己方消息状态位置 |
| `1c6a024` | 重排会话列表信息层级 |
...
消息免打扰完成后，后端跳过测试编译、SDK Core 与移动端 TypeScript 检查、`git diff --check` 均退出码为 0。入口调整后的共享设置组件和单聊设置页 ESLint 为 0；App、会话列表、群设置和聊天页的错误与警告数量均与调整前精确一致。按用户约定没有运行自动化测试。

会话列表时间与消息状态修复后再次执行相同静态检查。新增时间格式和状态组件 ESLint 为 0；会话列表保持既有 5 个 `no-void` 警告，聊天页保持既有 4 个 Hook 依赖错误且 `no-void` 警告由 21 个减少为 19 个。本轮没有新增 Lint 问题，也没有运行自动化测试。

常用 Emoji 面板完成后，移动端 TypeScript、文本编辑工具与面板组件 ESLint、`git diff --check` 均退出码为 0。聊天页仍精确报告既有 4 个 Hook 依赖错误和 19 个 `no-void` 警告，没有新增 Lint 问题；按用户约定没有运行自动化测试或模拟器视觉验证。

访问令牌自动刷新完成后，SDK Core 与移动端 TypeScript、相关认证文件 ESLint、`git diff --check` 均退出码为 0。`im-sdk-rn` 没有独立 `tsconfig.json`，其源码由移动端 TypeScript 检查覆盖；按用户约定没有运行自动化测试。
...
9. 会话列表时间与消息状态：代码已完成，等待用户手动验收。
10. 常用 Emoji 面板：代码已完成，等待用户手动验收。
11. 置顶聊天：前三项验收后开始设计。
...
1. 分别长按文本、图片、文件和语音并选择引用后发送；长按媒体时不应同时触发预览、打开或播放。
2. 分别点击图片、文件和语音，原有预览、打开和播放行为应保持正常。
3. 取消引用不清空文字；选择媒体或切换语音模式不消费引用。
4. 点击引用定位并短暂高亮本地原消息；本地缺失时显示提示。
5. 已撤回消息不能新引用；原消息后撤回时既有引用显示撤回占位且不泄露摘要。
6. 引用回复发送失败后重试，引用不丢失且最终只产生一条消息。
7. 单聊、群聊、双方实时接收、接收方离线恢复和页面重进后的展示一致。
8. 回复正文同时包含提及和链接时，正文高亮、引用块和链接卡片互不影响。
...
会话列表时间与消息状态设计和实施记录位于：

- `docs/superpowers/specs/2026-07-30-im-client-conversation-list-message-status-design.md`
- `docs/superpowers/plans/2026-07-30-im-client-conversation-list-message-status.md`

会话列表时间与消息状态手测需要覆盖：

1. 不同最新消息时间的会话显示各自正确时间，并按最新消息排序。
2. 今天、昨天、一周内、本年和跨年的格式正确；无消息会话不显示时间。
...
8. 撤回已落库消息时显示转圈，普通待发或失败消息不会被误判为撤回中。

常用 Emoji 面板设计和实施记录位于：

- `docs/superpowers/specs/2026-07-30-im-client-emoji-picker-design.md`
- `docs/superpowers/plans/2026-07-30-im-client-emoji-picker.md`

常用 Emoji 面板手测需要覆盖：

1. 点击输入框内笑脸，系统键盘收起并显示面板；再次点击切回系统键盘。
2. 面板显示后输入栏无重叠或横向溢出，添加、语音和发送按钮位置稳定。
3. 单击和快速连续点击 Emoji 均按顺序插入当前光标位置。
4. 选中文字后点击 Emoji，只替换选区并把光标放到新表情之后。
5. 退格可删除普通字符、变体 Emoji、肤色 Emoji、旗帜和家庭零宽连接 Emoji，不留下乱码。
6. 在 @提及内部插入、替换或删除时，失效绑定被清理；引用草稿保持不变。
7. 发送成功、切换语音、打开附件、切换会话、退后台或离开页面后面板关闭。
8. iOS 与 Android 上宫格可滚动，笑脸与退格按钮可点击并具有正确读屏标签。
...
当前分支是 feat/im，本轮提交尚未 push。语音消息、链接卡片、消息引用和富媒体长按已手动验收通过，消息免打扰、会话列表时间、消息状态布局和常用 Emoji 面板等待手动验收。
不要新增或运行测试；每完成一项功能，只告诉我需要手测哪些场景。
先检查工作区和最近提交，再从交接文档的“下一窗口优先处理的边界”继续。
先让用户按文档清单手测消息免打扰、会话列表时间、消息状态布局和常用 Emoji 面板；通过后设计置顶聊天。
