...
图片选择时自动压缩到最长边 2048、质量约 0.82；压缩后仍超过服务端 10 MiB 限制则提示重新选择。文件沿用服务端 100 MiB 上限。小于 5 MiB 的对象继续走现有单次预签名 PUT，达到 5 MiB 的对象走真正的 S3 Multipart Upload。上传任务持久化，应用重启或网络恢复后只补传缺失分片。应用进程被系统杀死期间不承诺继续上传。
